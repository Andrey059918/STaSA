...
const data = {
    round: {
        ...
        rSoft: document.getElementById('rSoft'),
        rMax: document.getElementById('rMax'),
        rMin: document.getElementById('rMin'),
        rCoeff: document.getElementById('rCoeff')
    },
    inputs: {
...
function Input_rMin() {
    let t = CheckInt(data.round.rMin);
    Calculate();
}

function Input_rMax() {
    let t = CheckInt(data.round.rMax);
    Calculate();
}

function Input_rCoeff() {
    let t = CheckInt(data.round.rCoeff);
    Calculate();
}
...
function Calculate() {
    ...
    var vLs = 0,
        ...
        vWq = 0;
    var vProb, vTmp;
    var rForce, rSoft, rMin, rMax, rCoeff;
    ...
    if (data.inputs.N.className == 'err' || data.inputs.S.className == 'err' ||
        ...
        if (vPsik >= 1 && vI) {
            ...
            rForce = data.round.rForce.checked;
            if (rForce && data.round.rMax.className == 'err') {
                rForce = false;
                layout.labels.warnRoundF.style.display = "block";
            } else {
                rMax = parseInt(data.round.rMax.value);
                layout.labels.warnRoundF.style.display = "none";
            }
            rSoft = data.round.rSoft.checked;
            if (rSoft && (data.round.rMin.className == 'err' || data.round.rCoeff.className == "err")) {
                rSoft = false;
                layout.labels.warnRoundS.style.display = "block";
            } else {
                rMin = parseInt(data.round.rMin.value);
                rCoeff = parseInt(data.round.rCoeff.value);
    ...
    data.outputs.Psi.value = advRound(vPsi, rForce, rMax, rSoft, rMin, rCoeff);
    data.outputs.Psik.value = advRound(vPsik, rForce, rMax, rSoft, rMin, rCoeff);
    data.outputs.P.value = advRound(vP, rForce, rMax, rSoft, rMin, rCoeff);
    data.outputs.Q.value = advRound(vQ, rForce, rMax, rSoft, rMin, rCoeff);
    data.outputs.An.value = advRound(vAn, rForce, rMax, rSoft, rMin, rCoeff);
    data.outputs.Aa.value = advRound(vAa, rForce, rMax, rSoft, rMin, rCoeff);
...
function advRound(num, force, decMax, soft, decMin, softCoeff) {
    var str;
    if (force) str = num.toFixed(decMax);
    else
        str = num.toString();
    if (soft) {
        let p0 = softCoeff,
        ...
        for (var i = dotPos + 1; i < str.length; i++) {
            if (str[i] == '0') {
                p0--;
                if (p0 <= 0 && i > dotPos + decMin) {
                    str = num.toFixed(i - dotPos - softCoeff + p0);
                    break;
                }
            } else if (str[i] == '9') {
                p9--;
                if (p9 <= 0 && i > dotPos + decMin) {
                    str = num.toFixed(i - dotPos - softCoeff + p9);
                    break;
                }
            } else {
                p0 = softCoeff;
